...
import corsOptions from './config/cors.js'; // Using optimized CORS
...
import { authRateLimiter, apiRateLimiter } from './middleware/auth.middleware.js'; // Using optimized middleware
...
// OPTIMIZATION: Configure trust proxy early
app.set('trust proxy', true);

// OPTIMIZATION: Compression first for all responses
app.use(compression());
...
// OPTIMIZATION: Conditional helmet configuration (lighter in development)
const helmetConfig = {
  ...
  } : false // Disable CSP in development for better performance
};
...
// OPTIMIZATION: Pre-compile webhook paths for faster checking
const WEBHOOK_PATHS = new Set(['/email-webhook', '/subscription/webhook']);

const isWebhookPath = (path) => {
  return WEBHOOK_PATHS.has(path) || path.includes('/webhook');
};
...
// OPTIMIZATION: Apply rate limiting more efficiently
...
app.use('/api', (req, res, next) => {
  // Skip rate limiting for webhook endpoints using optimized checking
...
// OPTIMIZATION: Conditional static file serving
if (process.env.NODE_ENV !== 'production') {
  app.use('/api/uploads', express.static('uploads', {
    maxAge: '1y', // Long cache for uploads
    etag: true,
    lastModified: true
  }));
...
// OPTIMIZATION: Conditional and lighter logging
const morganFormat = process.env.NODE_ENV === 'development' ? 'dev' : 'tiny';
app.use(morgan(morganFormat, {
  // Skip logging for health checks and static assets
  skip: (req) => req.path === '/health' || req.path.includes('/uploads/')
}));

// OPTIMIZATION: Conditional request URL logging (development only)
...
// OPTIMIZATION: Conditional API Documentation (development only)
if (process.env.NODE_ENV === 'development') {
