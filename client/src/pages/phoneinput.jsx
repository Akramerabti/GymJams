...
const PhoneInput = ({ 
  ...
  useEffect(() => {
    if (value) {
      // Try to detect country code and initialize correctly
      const digits = value.replace(/\D/g, '');
      
      // Handle E.164 formatted numbers (with + prefix)
      if (value.startsWith('+')) {
        ...
        for (const country of countryCodes) {
          if (digits.startsWith(country.code)) {
            ...
            setInputValue(digits.substring(country.code.length));
            break;
          }
        }
      } else if (digits.startsWith('1')) {
        // Default case for North American numbers
        setCountryCode('1');
        setCountryFlag('🇺🇸'); // Default to US
        setInputValue(digits.substring(1)); // Remove country code
      } else {
        // Just use the value as is if no country code detected
