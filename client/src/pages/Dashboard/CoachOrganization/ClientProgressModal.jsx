              ...
              <div className="mb-6">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="strength">Strength (Bench Press)</SelectItem>
                    <SelectItem value="cardio">Cardio (5K Run)</SelectItem>
              ...
              <div className="bg-gray-50 p-6 rounded-lg">
                <div className="mb-4">
                  <h4 className="font-semibold text-lg text-gray-800 mb-2">
                    {getMetricTitle(selectedMetric, progressData)}
                  </h4>
                  
                  {getMetricData(selectedMetric, progressData).length > 0 ? (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm text-left">
                        ...
                        <tbody>
                          {getMetricData(selectedMetric, progressData)
                            ...
                            .map((entry, index) => (
                              ...
                              >
                                ...
                                <td className="px-4 py-3 font-medium">
                                  {entry.value} {getMetricUnit(selectedMetric, progressData)}
                                </td>
                                ...
                                <Button
                                    variant="ghost"
                                    size="sm"
                                    onClick={() => handleDeleteEntry(selectedMetric, entry.date)}
...
const getProgressQuality = (progressData) => {
  // Placeholder logic for determining progress quality
...
const getProgressNotes = (progressData) => {
  // Placeholder logic for generating progress notes
  ...
  let notes = [];
  if (weightChange.value > 0) notes.push(`weight increased by ${weightChange.value.toFixed(1)} lbs`);
  if (strengthChange.value > 0) notes.push(`strength improved by ${strengthChange.value.toFixed(1)} lbs`);
  if (cardioChange.value > 0) notes.push(`cardio time improved by ${cardioChange.value.toFixed(1)} minutes`);
  if (bodyFatChange.value > 0) notes.push(`body fat decreased by ${bodyFatChange.value.toFixed(1)}%`);

  return notes.length > 0 ? ` Specifically, ${notes.join(', ')}.` : '';
};

const calculateOverallProgress = (progressData) => {
  // Placeholder logic for calculating overall progress
...
const calculateConsistency = (progressData) => {
  // Placeholder logic for calculating consistency
  const totalEntries = [
