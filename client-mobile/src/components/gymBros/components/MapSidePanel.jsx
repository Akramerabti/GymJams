...
import { 
  ...
  CheckCircle, TrendingUp, Award, Heart, Plus, 
  Loader, AlertTriangle
} from 'lucide-react';
import { DistanceUtils } from './ImageUtils';
import gymBrosLocationService from '../../../services/gymBrosLocation.service';
import { toast } from 'sonner';

const MapSidePanel = ({ isOpen, onClose, data, type, currentLocation, userProfile, onProfileUpdate }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Check membership status when panel opens for gyms
      if (type === 'gym' && data && userProfile) {
        checkMembershipStatus();
      }
    } else {
      setTimeout(() => setIsAnimating(false), 300);
    }
  }, [isOpen, data, type, userProfile]);

  if (!data && !isAnimating) return null;

  const calculateLiveDistance = () => {
    if (!currentLocation || !data?.lat || !data?.lng) {
      return { km: 0, miles: 0 };
    }
    
    const distanceKm = DistanceUtils.calculateDistance(
      currentLocation.lat,
      currentLocation.lng,
      data.lat,
      data.lng
  ...
  const checkMembershipStatus = () => {
    if (!userProfile || !userProfile.gyms || !data) return;
    
    const gymId = data._id || data.id;
    const membership = userProfile.gyms.find(g => 
      g.gym && g.gym.toString() === gymId.toString() && g.isActive
    );
    
    const isPrimary = userProfile.primaryGym && 
    ...
    setMembershipStatus({
      ...
      joinedAt: membership?.addedAt || null
    });
  };

  // Get current gym counts by type
  const getGymCounts = () => {
    if (!userProfile || !userProfile.gyms) {
      return { gymCount: 0, eventCommunityCount: 0 };
    }

    const activeGyms = userProfile.gyms.filter(g => g.isActive);
    
    // We need to get the actual gym data to check types
    // For now, let's assume we have the type in the gym association or use a reasonable split
    const gymTypes = ['gym', 'sport_center', 'other'];
    const eventCommunityTypes = ['community', 'event'];
    
    // Since we might not have the actual gym type data in the association,
    // we'll need to make an assumption or fetch this data
    // For now, let's count all as gyms and let the backend enforce the proper limits
    const gymCount = activeGyms.length;
    const eventCommunityCount = 0; // We'll need to implement this properly
    
    return { gymCount, eventCommunityCount };
  };

  // Check if user can join this facility
  const canJoinFacility = () => {
    if (!data) return { canJoin: false, reason: 'No facility data' };
    
    const facilityType = data.type || 'gym';
    const { gymCount, eventCommunityCount } = getGymCounts();
    
    // Define limits
    const GYM_LIMIT = 7;
    const EVENT_COMMUNITY_LIMIT = 14;
    
    const gymTypes = ['gym', 'sport_center', 'other'];
    const eventCommunityTypes = ['community', 'event'];
    
    if (gymTypes.includes(facilityType)) {
      if (gymCount >= GYM_LIMIT) {
        return { 
          canJoin: false, 
          reason: `You can only join up to ${GYM_LIMIT} gyms. Leave one first to join this one.` 
        };
      }
    } else if (eventCommunityTypes.includes(facilityType)) {
      if (eventCommunityCount >= EVENT_COMMUNITY_LIMIT) {
        return { 
          canJoin: false, 
          reason: `You can only join up to ${EVENT_COMMUNITY_LIMIT} communities/events. Leave one first to join this one.` 
        };
      }
    }
  ...
  const handleJoinGym = async () => {
    ...
    try {
      ...
      const { canJoin, reason } = canJoinFacility();
      ...
      // Determine default membership type based on facility type
      let membershipType = 'member';
      if (facilityType === 'community') {
        membershipType = 'volunteer';
      } else if (facilityType === 'event') {
        membershipType = 'guest';
      }
      
      // Check if this should be their primary gym (if they have no other gyms)
      const { gymCount } = getGymCounts();
      const shouldBePrimary = gymCount === 0;
      ...
      const result = await gymBrosLocationService.associateWithGym(
        ...
        membershipType,
        {
          ...
          preferredTimes: [],
          notes: `Joined via map on ${new Date().toLocaleDateString()}`
        }
      );
      ...
      if (result.success) {
        ...
        // Call onProfileUpdate to refresh user profile
        if (onProfileUpdate) {
          onProfileUpdate();
        }
      ...
      toast.error(error.message || 'Failed to join facility');
    } finally {
      setIsJoining(false);
    }
  };

  // Handle leaving a gym
  const handleLeaveGym = async () => {
    if (!data || !membershipStatus?.isMember) return;
    ...
    try {
      ...
      const result = await gymBrosLocationService.removeGymAssociation(gymId);
      ...
      if (result.success) {
        ...
        toast.success(`Left ${data.name} successfully`);
      }
    } catch (error) {
      console.error('Error leaving gym:', error);
      toast.error(error.message || 'Failed to leave facility');
    } finally {
      setIsJoining(false);
    }
  };
  ...
  const renderGymPanel = () => {
    const liveDistance = calculateLiveDistance();
    const { canJoin, reason } = canJoinFacility();
    ...
    return (
      <>
                ...
                {membershipStatus?.isMember && (
                  <div className="flex items-center gap-1 mt-1">
                    ...
                    <span className="text-xs text-green-300">
                      {membershipStatus.isPrimary ? 'Primary Gym' : 'Member'}
                    </span>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="bg-green-50 rounded-lg p-3 text-center">
                  ...
                  <p className="text-2xl font-bold text-black">
                    {currentLocation ? liveDistance.km.toFixed(1) : '0'}
                  </p>
                  <p className="text-xs text-black">km away</p>
                </div>
              </div>
              ...
              {membershipStatus?.isMember && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="font-medium text-green-900">You're a Member!</h4>
                  </div>
                  <div className="text-sm text-green-700 space-y-1">
                    <p>Membership: {membershipStatus.membershipType}</p>
                    {membershipStatus.isPrimary && (
                      <p className="font-medium">⭐ This is your primary gym</p>
                    )}
                    {membershipStatus.joinedAt && (
                      <p>Joined: {new Date(membershipStatus.joinedAt).toLocaleDateString()}</p>
                    )}
                  </div>
                </div>
              )}

              {/* Join Limitation Warning */}
              {!membershipStatus?.isMember && !canJoin && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <AlertTriangle className="w-5 h-5 text-red-600" />
                    <h4 className="font-medium text-red-900">Cannot Join</h4>
                  </div>
        ...
        <div className="p-4 border-t space-y-2">
          {membershipStatus?.isMember ? (
            // Already a member - show leave option
            <button 
              onClick={handleLeaveGym}
              disabled={isJoining}
              className="w-full py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {isJoining ? (
                <>
  ...
  // Keep existing renderUserPanel and renderEventPanel functions unchanged
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-sm">
                  {currentLocation 
                    ? `${liveDistance.km.toFixed(1)}km away`
                    : 'Distance unknown'
                  }
                </span>
              </div>
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                {currentLocation && data.lat && data.lng && (
